...
Manner_Web 是面向企业内部的 Web 管理系统：员工管理 + 角色权限（RBAC + 数据范围 + 部门角色继承）+ 聊天（单聊/群聊、文本/文件消息、拉黑）+ 系统设置。前后端分离：Rust(axum) REST API + SvelteKit 5 纯 SPA（adapter-static 静态产物）。
...
- **RBAC + 数据范围 + 部门角色继承模型**（方案 C，见 [权限系统重构方案.md](./权限系统重构方案.md)）：19 个权限码（employee 7 / department 5 / role 1 / chat 3 / system 2），权限经 `roles` 授予——员工直接分配（`employee_roles`）或部门绑定（`department_roles`），并沿 `parent_id` 继承；**员工级直接授权表 `employee_permissions` 已彻底移除**。
